...
description: çapraz etki alanı uygulamasını çivi altına almak için bir python web proxy sunucusu
gelişme. çapraz etki alanı için en önemli araçlardan biri
dirilişlerden beri uygulama araç kemeri n 'bok.
SEÇENEKLER:
  --host=HOST           proxy sunucusunu bağlamak için ana bilgisayar adı.
  --port=PORT           proxy sunucusunu bağlamak için bağlantı noktası.
  --max-threads         Maksimum aktif iplik sayısı.
  --log-level           çıkış seviyesini kaydetme
  --proxy-config        proxy yapılandırması yaml dosyası.
"""
...
class ProxyHandler(BaseHTTPRequestHandler):
  ...
  def render_pacfile(self):
    """
Proxy pacfile, proxy yapılandırmasında tanımlardan oluşur.
    """
    proxy_rule = """
    ...
    body = """
    fonksiyon FindProxyForURL(url, host) {
      {}
      return "DIRECT";
    }
  ...
  def urlparse(self, scheme, host, path, params, query, fragment):
    """
    urlparse () 'nin sonucunu bir dict olarak ayrıştırır.
    """
    return {
      'scheme': scheme,
      'host': host,
  ...
  def is_pacfile_path(self):
    """
    Geçerli istek yolu pacfile ise bir boolean döndürür.
    """
  ...
  def send_proxy_request(self, request, forward_proxy):
    ...
    if length > 0:
      ...
      self.server.logger.debug(
        "(content-length {}): ".format(length, body))
...
class ProxyContext(object):
  ...
  def on_max_threads(self):
    self.server.logger.warn(
      "maksimum etkin iş parçacığına ulaşıldı. sayımı sıfırla ..")
    # todo: Buraya gelmek için daha fazla eylem?
    self.active_thread_count = 0


  def set_sigint_handler(self):
    """
    'KeyboardInterrupt' olaylarını dinlemek için bir sinyali bağlar.
    """
  ...
  def sigint_handler(self, sigint, frame):
    """
    KeyboardInterrupt etkinlikleri için olay işleyici yöntemi.
    """
    self.server.logger.debug(
    ...
    while frame and isinstance(frame, FrameType):
      if frame.f_code and isinstance(frame.f_code, CodeType):
          # bu ProxyContext örneğini bulmak için yığından gider
  ...
  def on_sigint(self, code, msg, *args):
    ...
    self.server.logger.debug("on_sigint, kodu: {} msg: {}".format(code, msg))

    if code != 4 and not self.sigint_event.isSet():
      self.server.logger.critical("on_sigint hatası, kodu: {}, {}".format(code, msg))
      self.fail(msg)
...
parser = OptionParser(
  description="""description: çapraz etki alanı oluşturmak için bir python web proxy sunucusu
uygulama geliştirme.""",
  prog="pacman",
  usage="%prog [OPTIONS]")
...
parser.add_option(
  ...
  help="proxy sunucusunu bağlamak için ana bilgisayar adı.")

parser.add_option(
  "--port", dest="port", default=3128,
  help="proxy sunucusunu bağlamak için bağlantı noktası.")

parser.add_option(
  "--max-threads", dest="max_threads", default=1000,
  help="aktif ipliklerin maksimum sayısı.")

parser.add_option(
  "--proxy-config", dest="proxy_config", default="pacman.yaml",
  help="proxy pacfile yapılandırması dosyası.")

parser.add_option(
  "--log-level", dest="log_level", metavar="LEVEL",
  help="çıkış seviyesi kaydediliyor.")
